...
% Original game code by David Buckingham
...
%% Hero
COLLISION_DAMAGE = .3; %damage done to hero by collision with bot
HEALTH_RESTORE = .0015; %health restored each frame
...
FIRE_PROB = [.15, .04, .1]; %probability of firing each WAIT_FRAMES
...
%screen.
  function showIntro
    ...
    t(2) = text(x, titleY, 'FaceInvaders!');
...
%called when a bot leaves bounds or bot health drops to 0.
  function killBot(k)
    ...
    %clear plot
    delete(botPlots(k));
    delete(bots{k}.h_image);
    %remove plot from list of botPlots
    botPlots = [botPlots(:,1:k-1), botPlots(:,k+1:end)];
    %remove bot from list of botsqq
    
%     set(bots{k}.h_image,'Visible','Off')
...
%called by generateBots.
  function makeBot(model, startX, goalX)
    ...
    bots{end+1} = struct( ...
      ...
      'vector', botV, ...
      'burst', 0);
    picNumber = max(1,mod(length(bots),size(faces,1)));
%   display([length(bots) picNumber])
    bots{end}.h_image = image( ... 
        'CData',imresize(flipud(faces{picNumber,1}),1), ...
        'Parent',gca, ...
        'Visible','Off', ...
        'AlphaData',double(~faces{picNumber,2}));
...
f = load('FacesNew.mat');
faces = f.faces;
% aa = load('ava_audio.mat');
% y_ubmh = aa.y_ubmh;
% Fs = aa.Fs;
% aaa = load('ava_shop.mat');
% y_shop = aaa.y_shop;
% Fs1 = aaa.Fs;
...
while ~quitGame
  ...
  refreshPlots; %set data in all the plots
    ...
    pic_length_half = 8;
    pic_height_half = 8;
  for i = 1:min(length(bots),length(faces))
      set(bots{i}.h_image,'XData',[bots{i}.pos(1)-pic_length_half bots{i}.pos(1)+pic_length_half])
      set(bots{i}.h_image,'YData',[bots{i}.pos(2)-pic_height_half bots{i}.pos(2)+pic_height_half])
      set(bots{i}.h_image,'Visible','On')
  end
